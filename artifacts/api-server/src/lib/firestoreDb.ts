...
function toOrder(id: string, d: Record<string, unknown>): Order {
  return {
    ...
    trackingAvailable: (d.trackingAvailable as boolean) ?? false,
    shippedAt: fromTsOrNull(d.shippedAt),
    isGift: (d.isGift as boolean) ?? false,
    giftSenderName: (d.giftSenderName as string | null) ?? null,
    giftSenderPhone: (d.giftSenderPhone as string | null) ?? null,
    giftSenderEmail: (d.giftSenderEmail as string | null) ?? null,
    giftMessage: (d.giftMessage as string | null) ?? null,
    createdAt: fromTs(d.createdAt),
    updatedAt: fromTs(d.updatedAt),
  };
}
...
// Canonical product/size list — must match products.ts exactly (21 SKUs, 11 products)
const SEED_INVENTORY: Omit<InventoryItem, "id" | "updatedAt">[] = [
  ...
  { productName: "Beef Pickle",           sku: "KO-BP-100",  size: "100g",  batch: "", stock: 0, threshold: 10, incoming: 0, expiry: "", supplier: "", cost: 0 },
  { productName: "Beef Pickle",           sku: "KO-BP-250",  size: "250g",  batch: "", stock: 0, threshold: 10, incoming: 0, expiry: "", supplier: "", cost: 0 },
  ...
  { productName: "Nethili Sambal",        sku: "KO-NS-200",  size: "200g",  batch: "", stock: 0, threshold: 8,  incoming: 0, expiry: "", supplier: "", cost: 0 },
  ...
  { productName: "Soorai Pickle",         sku: "KO-SOP-250", size: "250g",  batch: "", stock: 0, threshold: 10, incoming: 0, expiry: "", supplier: "", cost: 0 },
  { productName: "Vaala Karuvaadu",       sku: "KO-VK-100",  size: "100g",  batch: "", stock: 0, threshold: 10, incoming: 0, expiry: "", supplier: "", cost: 0 },
  { productName: "Vaala Karuvaadu",       sku: "KO-VK-250",  size: "250g",  batch: "", stock: 0, threshold: 8,  incoming: 0, expiry: "", supplier: "", cost: 0 },
  { productName: "Maldives Fish Sambal",  sku: "KO-MFS-100", size: "100g",  batch: "", stock: 0, threshold: 10, incoming: 0, expiry: "", supplier: "", cost: 0 },
  { productName: "Maldives Fish Sambal",  sku: "KO-MFS-200", size: "200g",  batch: "", stock: 0, threshold: 8,  incoming: 0, expiry: "", supplier: "", cost: 0 },
...
function mapCouponDoc(doc: FirebaseFirestore.DocumentSnapshot): Coupon {
  const d = doc.data()!;
  return {
    id: doc.id,
    code: d.code,
...
export const couponsCol = {
  async findAll(): Promise<Coupon[]> {
    const snap = await fdb.collection("coupons").orderBy("createdAt", "desc").get();
  ...
  async create(data: Omit<Coupon, "id" | "createdAt" | "updatedAt">): Promise<Coupon> {
    const now = new Date();
    const ref = await fdb.collection("coupons").add({ ...data, createdAt: now, updatedAt: now });
    const doc = await ref.get();
    return mapCouponDoc(doc);
  },

  async update(id: string, updates: Partial<Omit<Coupon, "id" | "createdAt">>): Promise<void> {
    await fdb.collection("coupons").doc(id).update({ ...updates, updatedAt: new Date() });
  },
  ...
  async incrementUsed(id: string): Promise<void> {
    await fdb
      .collection("coupons")
      .doc(id)
      .update({ used: FieldValue.increment(1), updatedAt: new Date() });
  },
...
export const inventoryCol = {
  ...
  /** Upsert every canonical SKU — adds missing entries, leaves existing stock untouched.
   *  Returns counts of how many were added vs already present. */
  async syncAll(): Promise<{ added: number; skipped: number }> {
    const snap = await fdb.collection("inventory").get();
    const existingSkus = new Set(snap.docs.map(d => (d.data() as Record<string, unknown>).sku as string));
