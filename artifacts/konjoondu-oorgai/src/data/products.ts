import prawnImg from '@assets/Prawn_Pickle_250_1784776178703.png';
import chickenImg from '@assets/Chicken_Pickle_250_1784776178707.png';
import muttonImg from '@assets/Mutton_Pickle_250_1784776178707.png';
import beefImg from '@assets/Beef_Pickle_250_1784776178706.png';
import nethiliPickleImg from '@assets/Nethili_Pickle_250_1784776178706.png';
import nethiliSambalImg from '@assets/Nethili_Sambal_200_1784776178705.png';
import nethiliKaruvaduImg from '@assets/Nethili_Karuvaadu_Thokku_250_1784776178704.png';
import sooraiImg from '@assets/Soorai_Pickle_250_1784776178706.png';
import vaalaImg from '@assets/Valai_Karuvaadu_Thokku_250_1784776178704.png';
import maldivesImg from '@assets/Maldives_Fish_Sambal_200_1784776178705.png';
import idlyPodiImg from '@assets/Chinnakunni_podi_200_1784776178705.png';
...
export function getDisplaySize(product: Pick<Product, 'name' | 'tag'>): string {
  return /sambal|podi/i.test(`${product.name} ${product.tag}`) ? '200g' : '250g';
}

export function getPrimaryProductSize(product: Product): ProductSize {
  const displaySize = getDisplaySize(product);
  return product.sizes.find(size => size.label === displaySize) ?? product.sizes[product.sizes.length - 1];
}

export function getVisibleProductSizes(product: Product): ProductSize[] {
  const primarySize = getPrimaryProductSize(product);
  const smallSize = product.sizes.find(size => size.label === '100g');
  return smallSize && smallSize.label !== primarySize.label
    ? [smallSize, primarySize]
    : [primarySize];
}
