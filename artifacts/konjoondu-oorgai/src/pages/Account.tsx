...
import { LayoutDashboard, Package, User, MapPin, Heart, Bell, Star, Settings as SettingsIcon, LogOut, Menu, X, ShoppingBag } from 'lucide-react';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: 40, opacity: 0, scale: 0.95 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 40, opacity: 0 }}
        style={{ background: '#fffcf8', borderRadius: isMobile ? 20 : 28, padding: isMobile ? '24px 18px' : '36px 32px', width: '100%', maxWidth: 440, boxShadow: '0 40px 100px rgba(0,0,0,0.4)' }}>
    ...
    <div style={{ minHeight: '100vh', background: '#faf8f5', fontFamily: 'Poppins,sans-serif' }}>
      ...
      <div className="hidden md:flex" style={{ minHeight: '100vh' }}>
        ...
        <div style={{ marginLeft: 256, flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '20px 32px', borderBottom: '1px solid rgba(139,94,60,0.08)', background: '#fff', position: 'sticky', top: 0, zIndex: 5, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h1 style={{ fontSize: 22, fontWeight: 900, color: '#1a0f08', letterSpacing: '-0.02em' }}>{PAGE_TITLES[page]}</h1>
            <motion.button
              whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
              onClick={() => navigate('/products')}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 50, border: 'none', cursor: 'pointer', fontFamily: 'Poppins,sans-serif', fontWeight: 700, fontSize: 13, background: 'linear-gradient(135deg, hsl(4,65%,48%), hsl(4,60%,38%))', color: '#FFF9F0', boxShadow: '0 4px 14px rgba(181,58,46,0.3)', flexShrink: 0 }}>
              <ShoppingBag size={15} /> Order Oorgai
            </motion.button>
          </div>
      ...
      <div className="md:hidden" style={{ minHeight: '100vh', paddingBottom: 72 }}>
        {/* Mobile top bar */}
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(139,94,60,0.1)', padding: '12px 16px', position: 'sticky', top: 0, zIndex: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => setMobileSidebarOpen(true)} style={{ background: 'rgba(139,94,60,0.08)', border: 'none', borderRadius: 10, width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#4a3728', flexShrink: 0 }}>
            <Menu size={18} />
          </button>
          <h1 style={{ fontSize: 16, fontWeight: 900, color: '#1a0f08', flex: 1 }}>{PAGE_TITLES[page]}</h1>
          <motion.button
            whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
            onClick={() => navigate('/products')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 50, border: 'none', cursor: 'pointer', fontFamily: 'Poppins,sans-serif', fontWeight: 700, fontSize: 12, background: 'linear-gradient(135deg, hsl(4,65%,48%), hsl(4,60%,38%))', color: '#FFF9F0', boxShadow: '0 3px 10px rgba(181,58,46,0.28)', flexShrink: 0 }}>
