...
export default function AdminPage() {
  ...
  const fetchOrders = useCallback(async (tok: string): Promise<boolean> => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/orders`, {
      ...
      const data = await res.json();
      if (data.success) { setOrders(data.orders); return true; }
  ...
  useEffect(() => {
    if (token) {
      Promise.all([fetchOrders(token), fetchCustomers(token)]).then(([ordersOk]) => {
        if (ordersOk) setLoggedIn(true);
      });
    }
  }, []);
  ...
  async function saveShipment(orderId: string, courierPartner: string, trackingNumber: string) {
    try {
      const res = await fetch(`${API_BASE}/orders/${orderId}/shipment`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'x-admin-token': token },
  ...
  async function deleteShipment(orderId: string) {
    try {
      const res = await fetch(`${API_BASE}/orders/${orderId}/shipment`, {
        method: 'DELETE',
  ...
  function renderPage() {
    switch (page) {
      case 'dashboard':     return <Dashboard orders={orders} token={token} onRefresh={() => fetchOrders(token)} />;
      case 'orders':        return <Orders orders={orders} token={token} loading={loading} onRefresh={() => fetchOrders(token)} onUpdateStatus={updateStatus} onSaveShipment={saveShipment} onDeleteShipment={deleteShipment} onSeedDemo={seedDemo} seeding={seeding} />;
      ...
      case 'coupons':       return <Coupons token={token} onRefresh={() => {}} />;
      case 'analytics':     return <Analytics orders={orders} onRefresh={() => fetchOrders(token)} />;
      case 'reviews':       return <Reviews token={token} apiBase={API_BASE} />;
      case 'notifications': return <Notifications orders={orders} token={token} />;
      case 'settings':      return <Settings />;
      default:              return <Dashboard orders={orders} token={token} />;
    }
  }
