...
export default function AuthPage({ onSuccess }: { onSuccess?: () => void }) {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        ...
        setError('This domain is not authorised for Google sign-in. Add it to Firebase Console → Authentication → Settings → Authorised domains.');
      } else if (code === 'auth/operation-not-supported-in-this-environment') {
        setError('Google sign-in is not supported in this browser context. Try opening the app in a new tab.');
      } else {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: BG, padding: 20, position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Column wrapper: card + compliance text stacked vertically */}
      ...
      <motion.div
        ...
        style={{ width: '100%', borderRadius: isMobile ? 20 : 28, background: 'rgba(255,252,248,0.98)', boxShadow: '0 40px 100px rgba(0,0,0,0.5)', padding: isMobile ? '28px 20px' : '40px 36px' }}
      >
        {/* Header */}
      ...
      {/* reCAPTCHA ToS disclosure — below the card, centred */}
      <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(255,249,240,0.55)', marginTop: 14, lineHeight: 1.6, padding: '0 12px' }}>
        This site is protected by reCAPTCHA and the Google{' '}
        <a href="https://policies.google.com/privacy" target="_blank" rel="noreferrer" style={{ color: 'rgba(255,249,240,0.75)', textDecoration: 'underline' }}>Privacy Policy</a>
        {' '}and{' '}
        <a href="https://policies.google.com/terms" target="_blank" rel="noreferrer" style={{ color: 'rgba(255,249,240,0.75)', textDecoration: 'underline' }}>Terms of Service</a>
        {' '}apply.
      </p>

      </div>{/* end column wrapper */}

      {/* reCAPTCHA anchor — invisible widget attaches here */}
      <div id="recaptcha-container" style={{ position: 'absolute', bottom: 0, left: 0, visibility: 'hidden' }} />
