...
import { Package, MapPin, Heart, Bell, Star, Gift, TrendingUp, Clock, CheckCircle2, Truck, RefreshCw } from 'lucide-react';
...
function StatCard({ icon, label, value, color, onClick }: { icon: React.ReactNode; label: string; value: number; color: string; onClick?: () => void }) {
  ...
  return (
    <motion.div whileHover={{ y: -2, boxShadow: '0 12px 40px rgba(139,94,60,0.15)' }} transition={{ duration: 0.2 }}
      onClick={onClick}
      style={{ padding: isMobileCard ? '14px 10px' : '20px 18px', borderRadius: 18, background: '#fff', border: '1px solid rgba(139,94,60,0.1)', cursor: onClick ? 'pointer' : 'default', boxShadow: '0 2px 12px rgba(139,94,60,0.08)' }}>
      ...
      <div style={{ fontSize: isMobileCard ? 22 : 28, fontWeight: 900, color: '#1a0f08', lineHeight: 1 }}>{value}</div>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 4px' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}>
        <button onClick={load} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${PRIMARY}30`, background: 'transparent', color: PRIMARY, fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins,sans-serif' }}>
          <RefreshCw size={14} /> Refresh
        </button>
      </div>
      {/* Welcome card */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        style={{ borderRadius: 24, padding: isMobile ? '18px 16px' : '28px 28px', marginBottom: 24, background: `linear-gradient(135deg, ${PRIMARY} 0%, hsl(4,60%,30%) 100%)`, color: '#fff9f0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div>
              ...
              <h2 style={{ fontSize: isMobile ? 20 : 26, fontWeight: 900, letterSpacing: '-0.02em', marginBottom: 6 }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
        style={{ marginBottom: 24, padding: isMobile ? '16px 14px' : '22px 20px', background: '#fff', borderRadius: 20, border: '1px solid rgba(139,94,60,0.1)', boxShadow: '0 2px 12px rgba(139,94,60,0.06)' }}>
