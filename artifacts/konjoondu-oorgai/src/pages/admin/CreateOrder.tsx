...
import { Plus, Trash2, ShoppingBag, User, Phone, Mail, MapPin, CreditCard, CheckCircle, AlertCircle, ChevronDown, MessageSquare } from 'lucide-react';
...
const PAYMENT_METHODS = ['UPI', 'Net Banking', 'GPay', 'PhonePe'] as const;
...
export default function CreateOrder({ token, onSuccess }: { token: string; onSuccess: () => void }) {
  ...
  // Order is confirmed if: normal flow with paymentId, OR WhatsApp flow with paymentMethod + paymentId
  const isConfirmed = fromWhatsApp
    ? (paymentMethod !== '' && paymentId.trim() !== '')
    : paymentId.trim() !== '';
  ...
  return (
    <div style={{ padding: '28px 24px', maxWidth: 860, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: '#6b7c5a' }}>Record cash, phone, or WhatsApp orders directly in the system.</p>
      </div>

      {/* WhatsApp toggle */}
      <div style={{ marginBottom: 24, padding: '16px 20px', background: fromWhatsApp ? 'rgba(37,211,102,0.08)' : 'var(--adm-card, #fff9f5)', borderRadius: 14, border: `1.5px solid ${fromWhatsApp ? 'rgba(37,211,102,0.3)' : G.border}`, display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: fromWhatsApp ? 'rgba(37,211,102,0.15)' : 'rgba(139,94,60,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <MessageSquare size={18} color={fromWhatsApp ? '#25d366' : '#8b5e3c'} />
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 14, fontWeight: 700, color: fromWhatsApp ? '#25d366' : G.green, marginBottom: 2 }}>Order from WhatsApp</p>
          <p style={{ fontSize: 12, color: '#6b7c5a' }}>Toggle if this order came through WhatsApp — payment method details will be shown.</p>
        </div>
        <button
          type="button"
          onClick={() => { setFromWhatsApp(!fromWhatsApp); setPaymentMethod(''); }}
          style={{
            width: 48, height: 26, borderRadius: 13, border: 'none', cursor: 'pointer', position: 'relative',
            background: fromWhatsApp ? '#25d366' : '#d1d5db', transition: 'background 0.2s',
          }}>
          <span style={{
            position: 'absolute', top: 3, left: fromWhatsApp ? 25 : 3, width: 20, height: 20,
      ...
      <AnimatePresence>
        {fromWhatsApp && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div style={{ padding: '18px 20px', background: '#fff', borderRadius: 14, border: `1.5px solid ${G.border}` }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: G.mid, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14 }}>Payment Method (WhatsApp Order)</p>
                ...
                {PAYMENT_METHODS.map(m => (
                  <button key={m} type="button" onClick={() => setPaymentMethod(m)}
                    style={{
                      ...
                      border: `1.5px solid ${paymentMethod === m ? G.mid : G.border}`,
                      background: paymentMethod === m ? '#d1fae5' : 'transparent',
                      color: paymentMethod === m ? G.mid : '#6b7c5a',
                    }}>
                    {m}
                  </button>
                ))}
              </div>
              <Field label="Payment / Transaction ID" icon={CreditCard}>
                <input
                  style={{ ...inputStyle, borderColor: focusField === 'wp-pay' ? G.accent : G.border }}
                  value={paymentId} onChange={e => setPaymentId(e.target.value)}
                  onFocus={() => setFocusField('wp-pay')} onBlur={() => setFocusField('')}
                  placeholder="UPI transaction ID, order reference…"
                />
              </Field>
              {paymentMethod && paymentId.trim() && (
                <div style={{ padding: '10px 14px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #86efac', fontSize: 12, color: '#16a34a', fontWeight: 600 }}>
                  ✓ Payment confirmed via {paymentMethod} — order will be set to <strong>Confirmed</strong>.
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: `1.5px solid ${G.border}`, padding: 24 }}>
            ...
            <Field label="Full Name" required icon={User}>
              <input style={{ ...inputStyle, borderColor: focusField === 'name' ? G.accent : G.border }}
            ...
            <Field label="Phone" required icon={Phone}>
              <input style={{ ...inputStyle, borderColor: focusField === 'phone' ? G.accent : G.border }}
            ...
            <Field label="Email" icon={Mail}>
              <input style={{ ...inputStyle, borderColor: focusField === 'email' ? G.accent : G.border }}
            ...
            <Field label="Shipping Address" required icon={MapPin}>
              <textarea style={{ ...inputStyle, borderColor: focusField === 'addr' ? G.accent : G.border, minHeight: 80, resize: 'vertical' }}
            ...
            {/* Payment ID — only for non-WhatsApp orders */}
            {!fromWhatsApp && (
              <Field label="Payment ID" icon={CreditCard}>
                <input style={{ ...inputStyle, borderColor: focusField === 'pay' ? G.accent : G.border }}
                  value={paymentId} onChange={e => setPaymentId(e.target.value)}
                  onFocus={() => setFocusField('pay')} onBlur={() => setFocusField('')}
                  placeholder="pay_XXXX (leave blank for cash/pending)" />
                <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 5 }}>
                  Leave blank → <strong>Pending</strong>. Add payment ID → <strong>Confirmed</strong>.
                </p>
              </Field>
            )}
          </div>
                  ...
                  <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        <select value={item.productId} onChange={e => updateItem(idx, Number(e.target.value), item.size)}
            ...
            <div style={{ marginTop: 16, padding: '14px 18px', background: G.green, borderRadius: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2 }}>
                  Status: <strong style={{ color: isConfirmed ? '#86efac' : '#fcd34d' }}>
        ...
        <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          ...
          <motion.button type="submit"
            ...
            style={{
              ...
              cursor: submitting || !customer.name || !customer.phone || !customer.address ? 'not-allowed' : 'pointer',
